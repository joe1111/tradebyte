<?php
namespace ElasticExporttradebyteDE\Generator;
use ElasticExport\Helper\ElasticExportPriceHelper;
use ElasticExport\Helper\ElasticExportStockHelper;
use ElasticExporttradebyteDE\Helper\MarketHelper;
use ElasticExporttradebyteDE\Helper\PropertyHelper;
use ElasticExporttradebyteDE\Helper\StockHelper;
...
/**
 * Class tradebyteDE
 * @package ElasticExporttradebyteDE\Generator
 */
class tradebyteDE extends CSVPluginGenerator
{
    use Loggable;
    ...
    protected function generatePluginContent($elasticSearch, array $formatSettings = [], array $filter = [])
    {
        ...
        if($elasticSearch instanceof VariationElasticSearchScrollRepositoryContract)
        {
            ...
            do
            {
                $this->getLogger(__METHOD__)->debug('ElasticExporttradebyteDE::log.writtenLines', [
                    'Lines written' => $limit,
                ]);
                ...
                $this->getLogger(__METHOD__)->debug('ElasticExporttradebyteDE::log.esDuration', [
                    'Elastic Search duration' => microtime(true) - $esStartTime,
                ]);
                if(count($resultList['error']) > 0)
                {
                    $this->getLogger(__METHOD__)->error('ElasticExporttradebyteDE::log.occurredElasticSearchErrors', [
                ...
                if(is_array($resultList['documents']) && count($resultList['documents']) > 0)
                {
                    ...
                    foreach ($resultList['documents'] as $variation)
                    {
                        ...
                        {
                            $this->getLogger(__METHOD__)->info('ElasticExporttradebyteDE::log.variationNotPartOfExportStock', [
                        ...
                        {
                            $this->getLogger(__METHOD__)->error('ElasticExporttradebyteDE::logs.fillRowError', [
                    ...
                    $this->getLogger(__METHOD__)->debug('ElasticExporttradebyteDE::log.buildRowsDuration', [
                        'Build rows duration' => microtime(true) - $buildRowsStartTime,
                    ]);
                }
            } while ($elasticSearch->hasNext());
        }
        $this->getLogger(__METHOD__)->debug('ElasticExporttradebyteDE::log.fileGenerationDuration', [
    ...
    {
        $this->getLogger(__METHOD__)->debug('ElasticExporttradebyteDE::log.variationConstructRow', [
        ...
        if(!is_null($priceList['price']) && $priceList['price'] > 0 && $this->stockHelper->getStock($variation) > 0)
        {
            ...
            $this->getLogger(__METHOD__)->debug('ElasticExporttradebyteDE::log.variationConstructRowFinished', [
                'Data row duration' => 'Row printing took: ' . (microtime(true) - $rowTime),
            ]);
        }
        else
        {
            $this->getLogger(__METHOD__)->info('ElasticExporttradebyteDE::log.variationNotPartOfExportPrice', [
                'VariationId' => (string)$variation['id']
            ]);
        }
    }
